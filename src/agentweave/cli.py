...
def _build_agent_context(agent: str, session: "Session", version_comment: str) -> str:
    ...
    lines.append(f"<!-- Generated by {version_comment} — run `agentweave sync-context --force` to regenerate -->")
    lines.append("")

    # --- AgentWeave Protocol section ---
    lines.append("## AgentWeave Protocol")
    lines.append("")
    lines.append("You are participating in a multi-agent collaboration session managed by AgentWeave.")
    lines.append("")
...
def cmd_mcp_setup(args: argparse.Namespace) -> int:
    ...
    def _mcp_args(agent: str) -> list:
        ...
        runner_type = session.get_runner_config(agent).get("runner", "native") if session else "native"
        ...
        return [
            p.format(cli=cli, name="agentweave", server_cmd=server_cmd) for p in tpl
        ]
...
For more help: https://github.com/gutohuida/AgentWeave
    ...
    agent_configure.add_argument(
        "--runner",
        choices=["native", "claude_proxy", "kimi", "manual"],
