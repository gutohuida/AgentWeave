...
def cmd_activate(_args: argparse.Namespace) -> int:
    ...
    # Print run instructions for claude_proxy agents (need `agentweave switch` first)
    proxy_agents = [
        (name, agent)
        for name, agent in config.agents.items()
        if getattr(agent, "runner", "") == "claude_proxy"
    ]
    if proxy_agents:
        print("[PROXY] Claude-proxy agents — run commands:")
        for name, _agent in proxy_agents:
