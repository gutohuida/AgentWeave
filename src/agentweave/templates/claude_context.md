...
<!-- To regenerate from ai_context.md, run: agentweave sync-context --agent claude -->
<!-- To update with latest AI best practices, run: agentweave update-template --agent claude -->
...
2. **Load your role guide.** Try calling the `get_context` MCP tool with your role key
   (e.g. `get_context(role="tech_lead")`). If this succeeds, the returned content already
...
4. **If new or version changed and `get_context` was unavailable:** read the file at
   `roles.<role_key>.file` (e.g. `.agentweave/roles/tech_lead.md`) in full.
...
|-----------|---------------------------|
| API + Frontend | You: API implementation → Kimi: Frontend with mocks → Integrate when API ready |
| Feature + Tests | You: Core implementation → Kimi: Test scaffolding + edge cases |
| Multiple endpoints | You: Auth endpoints → Kimi: CRUD endpoints → Someone: Integration tests |
| Refactor + Feature | You: Refactor existing code → Kimi: New feature using stable API |
| Research + Build | You: Build with assumptions → Kimi: Research to validate assumptions |
...
- Unblocking others ("Fixed the API bug Kimi was waiting on")

**Principal's Duty: Keep the User Informed**
- Send a message at the start of major phases
- Notify when work is parallelized ("Kimi is building frontend while I do backend")
