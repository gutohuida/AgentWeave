...
<!-- To regenerate from ai_context.md, run: agentweave sync-context --agent gemini -->
<!-- To update with latest AI best practices, run: agentweave update-template --agent gemini -->
...
- Notify when work is parallelized ("Claude is building frontend while I do backend")
...
## Agent Skills (Gemini Native)

Gemini CLI supports Agent Skills via the `/skills` command. Unlike Claude's `.claude/skills/` directory, Gemini uses built-in and custom skills configured through the CLI.

### Built-in Skills to Enable

Create these via `/skills enable <name>` or configure in settings:
...
- **code-reviewer** — maintainability, complexity, naming, style

### Creating Custom Skills

Use `/skills add <name>` to create custom skills for specific project needs. Skills provide on-demand expertise and specialized workflows.

---

## Checkpointing & Session Management

Gemini CLI has built-in checkpointing for session management:

- **`/chat save <tag>`** — Save current conversation state
- **`/chat resume <tag>`** — Resume from a saved checkpoint
- **`/restore [tool_call_id]`** — Restore files to pre-tool state (undo edits)

**Best Practice:** Save checkpoints before major refactoring with `/chat save pre-refactor`
