...
**All twelve bundles reached R3 on 2026-09-24 (00:00–05:30) and are parked: 56 changes, all passing
`openspec validate --strict`.** What the operator does next, in order:

1. **Answer each bundle's "Open for the operator"** (its page's Final section) and record the answers in
   `DECISIONS.md`. Two rows are owed regardless: the 2026-08-29 decision to delete agent
   self-registration (B3; source handoff-0099 and ROADMAP), and F327-scope now being option (b) (B1).
2. **File or drop the candidate findings** each Final lists (not written to `FINDINGS.md`). Among
   them: the checkpoint worker runs `claude -p` without `--tools ""` on untrusted transcript text (B7);
   a read-only agent given a writing task writes into the operator's checkout (B5); approval surfaces
   answer a bare 500 when the gate's git calls raise (B5).
3. **Approve changes in `APPROVALS.md`**, respecting the build orders in the table and the
   migration note below. Several need `:8000` restarted before their UI bundle is committed (B9's
   F251, B12's F330, B4's Ask-me card retry).
