...
### A sixth — every refused firing creates a conversation

Found by counting rows rather than by looking for it. Across both probes, **five firings produced
five conversations, and three of those firings were refused**:

| Conversation | JobRun | Outcome |
|---|---|---|
| `conv-0ec4b8a6` | `run-68de7086` | fired, stranded, later reaped to `failed` |
| `conv-6eff6a98` | `run-81a0c68f` | **skipped** — "loop stop time reached" |
| `conv-dbb8d36a` | `run-52048b07` | **skipped** — "no claimable task among 1 open (1 blocked)" |
| `conv-0e2e0335` | `run-81dcbffe` | **skipped** — same |
| `conv-f86a1f49` | `run-c9991f8d` | fired for real; `run-424f7dda` completed, exit 0 |

The cause is ordering: `scheduler.py:818` calls `new_conversation(...)` and `name_conversation(...)`
long before the stop check (~868) and the stall check (~956-985) can return `False`. So a refusal
leaves behind a conversation named after the job, with nothing in it.

`agent-loops`' own requirement says a refused firing *"SHALL claim nothing, SHALL queue no input for
any agent, and SHALL leave every task's status and assignee unchanged"* — three things it must not
do, and creating a conversation is not among them. So this violates no requirement as written. It
does undercut one: **§626 exists because *"a loop left running fills the list with threads the
operator never began"***, and this is that, generated by firings that did nothing. On the 5-minute
cron this session moved the driver to, a stalled loop would produce **twelve empty threads an hour**,
each indistinguishable by name from a real one until opened.

Worth deciding rather than assuming: either create the conversation after the refusal points, or
state that a refused firing's conversation is expected and make it distinguishable in the list.

**All probe fixtures were removed:** both loops archived, both jobs archived, the probe agent
archived, its queue entry deleted, both probe tasks rejected, and `builder`/`speccer`/`verifier`
confirmed still bound to `runner-f654640b`. `GET /loops` returns zero active loops. The five
conversations remain — they are the evidence above, and nothing deletes a conversation.
