...
- [ ] 1.5 `hub/tests/test_task_transitions.py`: a source scan beside `test_only_the_binding_module_may_record_a_runtime_transition` — only `scheduler.py` and `task_transition_service.py` contain `ORIGIN_JOB`/`origin="job"`; and every `enter_selected_task(` call in `scheduler.py` passes `job_id=`. FAILS today (no such constant)
- [ ] 1.6 `GET /tasks/{id}/transitions` after 1.2's firing: rows in the route's own order (oldest first by `sequence`), the job row carries `job_id` and `job_name` equal to the job's name. FAILS today
- [ ] 1.7 UI (`hub/ui/src/__tests__/`): `TaskTransitionHistory` fed the route's real order renders `Flow <name> moved` for an `origin: 'job'` row and `You moved` for an `actor` operator row; a test reversing the two rows' order must still attribute each correctly (F190: the component must not rely on position). FAILS today
...
- [ ] 2.2 Model column + migration (nullable `String(64)`, no FK; head bumps per `.claude/rules/db-migrations.md`)
- [ ] 2.3 `enter_selected_task(..., job_id: Optional[str] = None)`: pass `origin=ORIGIN_JOB, job_id=job_id` to both `apply_transition` calls when set; `scheduler.py:3336` and `:3709` pass `job.id`; `agent_trigger.py:898` unchanged
- [ ] 2.4 `_transition_view` adds `job_id`, `job_name`; MCP `task_history` docstring names the origin
