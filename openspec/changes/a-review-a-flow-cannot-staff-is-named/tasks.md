...
- [ ] 1.5 **D8's reproduction.** Move (b) to `under_review` by hand with the working agent still in
...
- [ ] 2.4 Add `agents_that_worked(session, task_id) -> set[str]` beside it: `SELECT DISTINCT
  ...
  restaff, and a task returned for revision has two authors.
- [ ] 2.5 Test `agents_that_worked` on all three fixtures: (a) returns the agent, (b) returns the
  agent that worked it even though the operator completed it, (c) returns the empty set.
...
- [ ] 3.3 The *operator completed it* arm resolves through the ladder with
  `exclude=await agents_that_worked(session, task.id)`. Nothing else about the call changes:
  `unavailable=taken` still, `choice.rung` handled exactly as today.
...
- [ ] 4.1 `task_is_claimable_by` (`scheduler.py:546-593`) gains the operator arm: claimable by any
  agent not in `agents_that_worked`. The *nothing recorded* case keeps returning `False` unchanged.
...
- [ ] 5.1 `scheduler.py:1279-1284`: where no agent is recorded as completing the task and
  `task.assignee` is in `agents_that_worked`, set `wedged_review = True` rather than appending to
  `in_flight`. Where a completion names an agent, behaviour is unchanged.
...
- [ ] 5.4 Test the case that must **not** change: a task in `under_review` with a legitimate reviewer
  ...
  reports every review in progress as unstaffable is worse than the bug.
