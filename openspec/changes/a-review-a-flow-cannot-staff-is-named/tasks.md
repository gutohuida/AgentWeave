...
- [x] 1.1 New file `hub/tests/test_a_flow_names_what_it_cannot_staff.py`. Do not grow
...
- [x] 1.2 **The fixture is the hard part, and getting it wrong makes every assertion below
...
- [x] 1.3 **F142's reproduction.** Fire `decide_firing` on a queue holding only (b) and assert
...
- [x] 1.4 **The self-approval reproduction, which is the one that matters (design D5).** On queue
...
- [x] 1.5 **Round 2's reproduction, and it is the one that would have shipped a self-approval.**
...
- [x] 1.5a **Round 3's reproduction, and it is the one round 2's repair does not close.** Fixture
...
- [x] 1.6 **D8's reproduction.** Move (b) to `under_review` by hand with the working agent still in
...
- [x] 2.1 Add `CompletionAttribution` (frozen dataclass: `recorded: bool`, `actor_kind:
...
- [x] 2.2 Make `agent_that_completed` a wrapper returning `attribution.agent`, with its signature and
...
- [x] 2.3 Test the invariant D2 rests on, rather than assuming it: for a `→ completed` row,
...
- [x] 2.4 Add `agents_that_worked(session, task_id) -> set[str]` beside it: `SELECT DISTINCT
...
- [x] 2.4a Add `agents_of_runs_bound_to(session, task_id) -> set[str]`: `SELECT DISTINCT agent FROM
...
- [x] 2.5 The exclusion the ladder is given is `agents_that_worked(...) | ({task.assignee} if
...
- [x] 2.6 Test `agents_that_worked` on all five fixtures: (a) returns the agent, (b) returns the
...
- [x] 3.1 Replace `scheduler.py:1364-1368` with the three-way split. Keep the gate order: the
  ...
  sentence.
- [x] 3.2 The *nothing recorded* arm appends to `unstaffed`, naming the task and the remedy. Wording
...
- [x] 3.3 The *operator completed it* arm resolves through the ladder with the 2.5 union as
  ...
  group 3a.
- [x] 3.3a **Round 2's self-approval reproduction inverts.** On fixture (d), with the agent that did
...
- [x] 3.3b **Round 3's inverts too, and it is the one a two-term exclusion passes.** On fixture (e),
...
- [x] 3.4 The *an agent completed it* arm is byte-identical to today, `exclude={author}`. Design D6:
  where the product has a decided answer to who the author is, do not widen it.
- [x] 3.5 Test each arm against its fixture, and test that the operator arm reaches an *approved*
...
- [x] 3.6 Test the exclusion actually excludes: queue (b), two agents, one of which worked the task.
...
- [x] 3a.1 `resolve_reviewer` gains `excluded_because: str = "is the one that completed this task"`
...
- [x] 3a.2 The operator-completed arm passes `excluded_because="has worked on this task"`. The
  attributed arm passes nothing and keeps the default.
- [x] 3a.3 Test both sentences on both arms, asserting the **absence** of the word `completed` in the
...
- [x] 3a.4 Grep for existing assertions on either sentence before changing them
...
- [x] 4.1 `task_is_claimable_by` (`scheduler.py:546-593`) gains the operator arm: claimable by any
...
- [x] 4.2 Extend the docstring's *argument* — *"handing finished work to an agent the Hub cannot rule
...
- [x] 4.2a **And correct the two sentences in that docstring that are false** (design D17). Not
  ...
  agent.
- [x] 4.3 Add the cases — (b), (d) and (e) — to `test_actor_aware_claimability.py` beside its existing
  ...
  (c), not case (b).
- [x] 4.4 Check `_first_startable_candidate` and the board summary now agree with the flow walk on
...
- [x] 5.1 `scheduler.py:1279-1284`: where no agent is recorded as completing the task and
  ...
  catches it.
- [x] 5.2 Confirm the carry-through: a wedged case (c) reaches the ladder and comes back `unstaffed`
...
- [x] 5.3 Test: 1.5's reproduction inverts — case (b) in `under_review` with its worker as assignee
...
- [x] 5.4 Test the case that must **not** change: a task in `under_review` with a legitimate reviewer
...
- [x] 6.1 No code change is expected — `scheduler.py:1438-1457` already substitutes `unstaffed[0][1]`
...
- [x] 6.2 Test the persisted event and the SSE payload, not only the stall string. F64's own history
...
- [x] 6.3 Check the `DECISION_PROCEED_EMPTY` path: a queue whose only task is case (b) or (c) and
...
- [x] 7.2 Add row four: the operator completes a task **no agent ever touched**, and a review is
  staffed with nobody excluded. That is the arm with the widest exclusion behaviour and it has no
  drive coverage.

## 8. Verify

- [x] 8.1 `py -3.11 -m pytest hub/tests/test_a_flow_names_what_it_cannot_staff.py
...
- [x] 8.2 `py -3.11 -m pytest hub/tests/ -q -k "flow or loop or review or transition or scheduler or
  claim"`.
- [x] 8.3 **`test_flow_chain_end_to_end.py:344-355`'s set equality must be unchanged.** This change
...
- [x] 8.4 `ruff check src/ hub/ tests/` and
  `black --check --target-version py311 src/ hub/hub/ hub/tests/ tests/`.
- [x] 8.5 `openspec validate a-review-a-flow-cannot-staff-is-named --strict`.
