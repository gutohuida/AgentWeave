...
Under the posture in which the Hub decides each tool call against the run's workspace, each word of a shell command that is a path SHALL be judged by where it resolves against the run's workspace, and a refusal SHALL name the path as the command wrote it.
...
that begins with a variable, or with the home-directory shorthand, and continues as a path. Such a
word SHALL be refused with a reason saying it cannot be checked. It SHALL NOT be refused with a
reason claiming it is outside the workspace, and it SHALL NOT be allowed on the grounds that nothing
visible in it points outside.
...
Where the platform accepts more than one path separator, each is a separator.
...
- **WHEN** a shell command names a relative path that resolves outside the workspace after
  traversal
- **THEN** the command is refused
- **AND** the reason names the path as the command wrote it

#### Scenario: A path the shell builds when it runs is refused as uncheckable

- **WHEN** a word of a shell command begins with a variable or with the home-directory shorthand and
  continues as a path
- **THEN** the command is refused
...
- **WHEN** the platform accepts more than one path separator, and a relative path written with
  either of them resolves outside the workspace
- **THEN** the command is refused
