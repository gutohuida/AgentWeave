...
- **A Settings section on the loop's own tab.** It shows and edits name, default agent, message,
  ...
  work needs evidence").
- **The default agent becomes editable.** `JobUpdate` gains `agent`, checked by the existing
  `_check_agent_exists`. On a loop it is **staged** like purpose and stop condition (new
  `Loop.pending_agent`, a migration) and applied at the next firing. On a plain job it applies at
  once and clears `last_session_id`, since the resumed session belonged to the old agent.
- **A loop's listing entry names the document it declares.** `LoopSummary` gains
  `spec_document_id`, so a document can find its flow.
...
- `job_updated` also refreshes the loop caches, so another open window sees an edit.
...
- `agent-loops`: "An edit to a loop takes effect at its next firing and never during one" names the
  ...
  a loop's settings from the loop's own view".
...
- **Backend:** `schemas/jobs.py` (`JobUpdate.agent`), `api/v1/jobs.py` (PATCH agent handling,
  `LoopSummary.spec_document_id`, `_pending_loop_edit`), `scheduler.py` (`_stage_pending_loop_edit`
  applies `pending_agent`), `db/models.py` plus one migration (`loops.pending_agent`, nullable).
- **UI:** `api/jobs.ts`, `api/loops.ts`, `LoopTab.tsx`, a new `StartFlowDialog.tsx`,
  `SpecPhaseBar.tsx`, `useSSE.ts`, and a bundle refresh.
- **Order:** after B10's `a-loop-is-stopped-archived-and-delegated-from-its-own-tab` (same tab, same
  hooks file, same PATCH route). Before change 2.
- **`:8000`:** the bundle reaches the operator's live app on their next reload. Until `:8000`
  restarts, an agent edit from that page answers 422 and changes nothing (the old route), and every
  other field works.
