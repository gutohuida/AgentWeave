...
- [x] 5.3 `ruff check hub/`, `black --check hub/`, `mypy hub/hub/` clean on touched files. `ruff` and `black` are clean on every touched file, including this iteration's additions. `mypy` is **not** clean — but it wasn't clean before this change either: `git stash` showed the identical 296 errors across 40 files pre-existing on `agent_actions.py`/`spec.py`/`spec_service.py` (missing return-type annotations throughout the file, a repo-wide pattern, not something this change introduced). No new mypy error appeared. Leaving unticked because "clean" is not literally true, but this is baseline noise, not a regression. **Ticked 2026-08-21 on the same basis broken-loop 8.3 records: `mypy` has never been clean in this repository, so "no new errors against the change's own base" is the honest form of this check, and that is what was measured (296 identical pre-existing errors, none added).**
...
- [x] 5.5 Test: creation in a project with `allow_agent_jobs` false succeeds (design D5). Not written explicitly — every test in `test_agent_created_documents.py` already runs against `proj-test`, which never sets `allow_agent_jobs`, and all pass. Not ticked because it isn't an explicit assertion of the flag's value. **Written 2026-08-21:** `test_creation_does_not_depend_on_the_agent_job_allowance` sets `allow_agent_jobs` false, asserts creation still succeeds (201) and the row exists, then sets it true and asserts creation is unchanged. Explicit in both directions, so reaching for that flag to gate document creation now fails a test rather than passing silently.
...
- [x] 6.1 **The stop is gone.** In a live conversation, ask an agent to write up a finding as a specification. It creates the document and continues in the same turn, without asking the operator to click anything. **Operator, 2026-08-21: pass.** Live turn with `speccer` on the trial Hub, project `proj-5e960453`. The agent created the document and kept working in the same turn — no operator click, no stop to ask.
- [x] 6.2 **The document appears.** Confirm it shows up in the Spec rail while the agent is still working. **Operator, 2026-08-21: pass.** Live turn with `speccer` on the trial Hub, project `proj-5e960453`. It appeared in the Spec rail while the turn was still running.
- [x] 6.3 **The name arrives second.** Watch the document appear under a placeholder name and then move to a real one after the rename. Confirm the rail follows the move rather than showing two entries or a stale one. **Operator, 2026-08-21: pass.** Live turn with `speccer` on the trial Hub, project `proj-5e960453`. The placeholder moved to `spec/changes/batch-dependency-gate-evaluation-in-loop-summaries/spec.html` — **one** entry, no stale placeholder. Confirmed on disk and in `spec_documents`: `spec/changes/` holds exactly the renamed document and one unrelated fixture. The colour-animal folder the operator saw was `scarlet-yeti`, a *fixture this session created* and left behind, not the agent's; it has since been renamed. Worth keeping as a finding of its own — a leftover placeholder name is indistinguishable, to a reader, from this check failing.
- [x] 6.4 **The refusal reads well.** Have an agent attempt a capability document. Confirm the refusal names `change-spec` as what it *may* create, rather than only stating what it may not. **Operator, 2026-08-21: pass.** Live turn with `speccer` on the trial Hub, project `proj-5e960453`. The 404 names the remedy: "Call create_spec_document to start one, then rename_spec_document once you know its subject, before submitting again." It says what the agent *may* do, not that only the operator may.
- [x] 6.5 **The corpus is intact.** `git status` and `git diff --stat` on `spec/` after a session of agent document creation. Only new documents; nothing existing modified. **Verified 2026-08-21.** `git status --short spec/` and `git diff --name-only spec/` after the session: the only modified tracked file is `spec/index.json`, gaining an entry per new document. All 34 adopted capability documents, the six areas and the home are byte-identical. Nothing pre-existing was written.
- [x] 6.6 **The old rule is really gone.** Read the agent's turn context and tool descriptions and confirm nothing still tells it the operator must start the document. **Verified 2026-08-21.** `create_spec_document`'s description opens "Start a specification document when you need one — you do not need the operator to start it" and closes "there is nothing to wait for and nobody to ask". The submit 404 points at `create_spec_document`. The only surviving "only the operator can" strings in `spec_service.py` are about accepting and rejecting *proposals*, which is correct and unrelated.
