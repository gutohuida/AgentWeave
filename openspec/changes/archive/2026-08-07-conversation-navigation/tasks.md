...
- [x] 7.5a The pre-selected agent is a **default, not a lock** — the roster stays live and retargets the unsent message (operator, 2026-08-08: *"I like the way that it pre select but it shouldn't lock. If I change the conversation should be directed elsewhere. Or we should remove the other agents from the screen so it's not confusing"*). Shipped broken in `b947d87`: `agent = boundAgent ?? chosen` rendered every chip clickable and then ignored the click. Rejected the operator's second option (hide the other agents when pre-bound) — it makes the surface's shape depend on how you arrived at it, and forbids a change of mind that costs nothing before the first message. The chosen agent moved into the destination for the same reason the open conversation did in §8: two sources of truth is what let the pre-selection outrank the click
