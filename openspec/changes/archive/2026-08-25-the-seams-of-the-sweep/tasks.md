...
- [x] 8.6 **Driven live.** `job-e2c18b2d` disabled first at the operator's direction (36 firings, last 13:00), the 8010 instance restarted from source on the new code, and `GET /health` confirmed `{"status":"ok","runtime":"native"}` — the `runtime` field is new in this change, so it is the cheapest proof the restart took. Eleven of the twelve findings verified against `proj-bacb623ca9ba`, F27 by a real agent run refused 403 at the exact call. **F38 did not verify, and that is the result**: every precondition was met and nothing was recorded — see F41
- [x] 8.7 Recorded in `scripts/drive/FINDINGS.md` under "The live re-drive, 2026-08-25" — a row per finding, including the one that did not reproduce as expected (F38) and the two the drive produced (**F41**, F38's fix is unreachable in production, fixed here; **F42**, what F27's guard does and does not close). The operator's dispositions on F22/F24/F25/F26/F35/F39/F40 are recorded in the same section
- [x] 8.8 Reconciled: 18 requirements added across 10 capabilities, 1 replaced in place (`agent-stream-events` `Structured payload safety`, still at position 8 with its original scenarios intact), and `openspec/specs/turn-outcome-visibility/spec.md` created as a new capability. `git diff --stat` over `openspec/specs/` is 497 insertions and 0 deletions, so nothing pre-existing was displaced. `npx openspec validate --all --strict` passes 42 of 42. Archived to `openspec/changes/archive/2026-08-25-the-seams-of-the-sweep/`
