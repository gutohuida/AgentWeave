...
| The product, end to end | the scratch-Hub drive (tasks 4.1, 4.2) | B's CLI transcript has no file attachment for a peer's `@<outside path>`; the operator's `@<inside path>` still attaches |
