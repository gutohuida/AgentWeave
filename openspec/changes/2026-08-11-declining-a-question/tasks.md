...
- [ ] 1.1 `questions.declined` and `questions.declined_at` in `hub/hub/db/models.py`, beside
      `answered` rather than replacing it (D1)
- [ ] 1.2 Migration `0061`, guarded, no backfill; bump **both** head assertions
- [ ] 1.3 `POST /questions/{id}/decline`, refusing a question that is already answered
- [ ] 1.4 `declined` on `QuestionResponse`; the decline is broadcast and persisted as an event
- [ ] 1.5 Tests: declining closes it; an answered question cannot be declined; declining twice is
      idempotent

## 2. The agent is told

- [ ] 2.1 `ask_user`'s wait ends on answered **or** declined, and reports which (D2)
- [ ] 2.2 `get_answer` reports a declined question as declined rather than as still pending
- [ ] 2.3 The `ask_user` docstring says the operator may decline, and what that means — this is a
      change to what the agent is told, not a note about one
- [ ] 2.4 Tests: the poll ends on a decline; the returned entry says `answered: False` and names the
      decline

## 3. A declined question does not hold a task

- [ ] 3.1 `unanswered_blocking_question` excludes declined rows (D4) — without this the run-boundary
      check re-parks the task on a question the operator already closed
- [ ] 3.2 Declining releases a task the question parked, through the **same** function answering
      uses (D3)
- [ ] 3.3 Tests: a declined question parks nothing at the run boundary; declining releases a parked
      task to `in_progress` and clears its reason; the check applies again afterwards

## 4. The operator's surface

- [ ] 4.1 `QuestionResponse.asker_waiting`, computed from the asking run and defaulting to `True`
      when unknown (D5)
- [ ] 4.2 `activeQuestionFor` skips declined questions and sorts live ones first (D6)
- [ ] 4.3 A dismiss control on `AgentQuestionCard`
- [ ] 4.4 The card marks a question nobody is waiting on
- [ ] 4.5 Tests: a declined question is never the active one; a live question outranks a stale one;
      **a batch stays contiguous under the new sort** — the property D6 depends on
- [ ] 4.6 `npm run build`, `rm -rf hub/hub/static/ui`, copy, confirm with `diff -rq`

## 5. Specs

- [ ] 5.1 Apply the two delta specs
- [ ] 5.2 `npx openspec validate --specs --strict` and `--changes --strict`

## 6. Verification

### 6a. Agent-verifiable

- [ ] 6.1 `pytest hub/tests/ -q` and `pytest tests/ -q` — all pass, counts no lower than at start
- [ ] 6.2 `cd hub/ui && npx vitest run`, `npx tsc --noEmit` — pass and clean
- [ ] 6.3 `ruff check` clean; `black` applied
- [ ] 6.4 Unit: declining ends the poll rather than running to the deadline
- [ ] 6.5 Unit: a declined question neither parks a task nor keeps one parked
- [ ] 6.6 Unit: a batch does not fragment under the live-first sort
- [ ] 6.7 Live: behavioural probe against a database copy; confirm the serving process is new by
...
- [ ] 6.9 With a stale question and a live one outstanding, is it obvious which one is being asked
      of you?
- [ ] 6.10 Is "nobody is waiting on this" the right thing to say about a question whose agent has
      moved on — or does it read as an error?
- [ ] 6.11 Does declining ever lose something you wanted back? (D1 keeps the record; nothing surfaces
