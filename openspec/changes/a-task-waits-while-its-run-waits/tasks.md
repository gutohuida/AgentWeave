...
- [ ] 2.6 Correct the **four** comments that now state something false — round 1 counted two. `hub/hub/api/v1/tasks.py:336-343` (`_attach_awaiting_answer`'s docstring), `hub/hub/schemas/tasks.py:315-322` (`awaiting_answer_reason`'s comment), `hub/ui/src/api/tasks.ts:34-43` (the same sentence on the TypeScript field) and `hub/ui/src/components/tasks/TaskCard.tsx:115-119` ("does not park its task until it *ends*"). All four say a task reaches `blocked` only when the asking run ends. Say what the field is still for instead — the two cases in design D9.
- [ ] 2.6a No UI *behaviour* change is needed for half (a), and the reason is worth a test rather than a comment: `TaskCard` already coalesces (`const waitingReason = task.blocked_reason ?? awaitingAnswer`, `isWaitingOnOperator = isBlocked || Boolean(awaitingAnswer)`), so a task that is now both `blocked` and carrying `awaiting_answer_reason` renders one wait, not two. Assert it in `hub/ui/src/__tests__/taskBlockedTreatment.test.tsx`.
...
- [ ] 9.5 `openspec validate a-task-waits-while-its-run-waits --strict`, then sync **both** deltas — `openspec/specs/task-lifecycle-governance/spec.md` and `openspec/specs/task-dependencies/spec.md` — and archive the change. `--strict` does not compare capabilities, so it will not tell you if one is missed; check by grepping the archived corpus for "Resuming is gated" and finding nothing.
