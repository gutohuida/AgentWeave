...
- [ ] 1.5 Codex: `_await_operator_permission(..., workspace=<ws>)` with a subject whose `cwd` is outside stores `allow: False`. FAILS today
...
- [ ] 2.4 `codex_appserver.workspace_verdict` and `_await_operator_permission(workspace=…)`
