...
- [x] 1.1 New file `hub/tests/test_loop_lands_its_work.py`. Not `test_task_integration.py` (which is
  about the evidence route and is already 900 lines) and not `test_loop_*.py` (which are about
  scheduling).
- [x] 1.2 **Reuse the fixture, do not build one.** `hub/tests/test_task_integration.py` has
...
- [x] 1.3 **The reproduction is a loop task with real work on its own branch.** Create a loop
...
- [x] 1.4 Drive it to `approved` and assert **today's** behaviour: the transition succeeds, and a
...
- [x] 1.5 Reproduce break 7 in the same file at the API level: `POST
...
- [x] 1.6 Reproduce break 7 in the UI. `hub/ui/src/__tests__/taskIntegrationRetry.test.tsx` exists;
...
- [x] 1.7 The guards that must still pass afterwards, written now as assertions about today: a task on
...
- [x] 1.7a **The guard round 3 added, and the one that would have caught D11** — the same shape one
...
- [x] 2.1 `Loop.work_needs_evidence: Mapped[Optional[bool]] = mapped_column(Boolean, nullable=True)`
...
- [x] 2.2 Migration `hub/hub/migrations/versions/0100_loop_work_needs_evidence.py`, `down_revision =
...
- [x] 2.4 Add the `0100`-is-guarded test beside `test_migration_0098_is_guarded_when_the_queue_table_does_not_exist`
...
- [x] 4.1 In `hub/hub/task_integration.py`: `NO_TASK_BRANCH = "this task has no branch of its own, so
  there is nothing to merge"`, beside the other reason constants.
- [x] 4.2 `def task_branch_tip(root: Path, task_id: str) -> Optional[str]` — `git rev-parse --verify
  ...
  not mint.
- [x] 4.3 `async def merge_targets(session, task, root) -> List[Target]` — design D5, **D10 and
  ...
  **`integration_targets` is not modified.** The branch tip is the answer for exactly one
  ...
  describes.
- [x] 4.3c `async def task_has_requirement_links(session, task) -> bool` in
...
- [x] 4.3a Use `session.get(Loop, task.loop_id)`, not `select(Loop).where(...)`. `_merge_situation`
...
- [x] 4.3b Tests for the resolver itself, and the first is the one that matters: **a flow task with
...
- [x] 4.3d **The test that would have caught D11**, and it is the one that matters most here: a task
...
- [x] 4.4 `hub/hub/task_transition_service.py` `integrate_task`: `merge_targets(session, task, root)`
...
- [x] 4.5 Do **not** touch the ordering of `release_task_workspace` relative to `integrate_task`
...
- [x] 4.7 `task_integration_preview` (`hub/hub/api/v1/tasks.py:1039-1089`) resolves a root and calls
...
- [x] 4.8 Test: the preview for an evidence-free loop task names the branch-tip commit and its
  branch, and the preview for a loop that declares evidence is needed is unchanged.
- [x] 4.6 Tests: the branch tip merges; a second commit made after approval does not; a grandfathered
...
- [x] 4.9 **`_prerequisite_commits` asks the same question — design D12, the decision round 2 left
  ...
  **Why this is in scope and not late widening: without it the change breaches a shipped
  ...
  must land with it.
- [x] 4.9a Test: two evidence-free loop tasks, A → B, A approved while the primary checkout is
...
- [x] 4.9b Test the cost, so it is a known behaviour rather than a surprise: a prerequisite whose
...
- [x] 4.9c Test the guard: an **unapproved** prerequisite on an evidence-free loop contributes
...
- [x] 5.1 `hub/hub/requirement_gate.py` `_merge_situation`: build the target list with `merge_targets`
...
- [x] 5.2 Check `_check_unaccepted` reads correctly after the rename: its `if situation.accepted:`
...
- [x] 5.3 Test: an evidence-free loop task whose branch conflicts with the main branch is **refused**
  ...
  the evidence route.
- [x] 5.4 Test: an evidence-free loop task in a project with no configured main branch approves
