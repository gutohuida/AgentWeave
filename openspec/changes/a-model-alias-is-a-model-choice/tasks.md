...
- [ ] 0.1 R2: re-derive the proposal independently. Grep every caller of `ProviderDescriptor.model`, `get_provider` and `model_is_declared`, and every place a stored `Runner.model` is compared or rendered (including `hub/ui/src`). Confirm design D1's table is complete. Record the result in `design.md`'s round log
...
- [ ] 2.4 UI: `RunnerForm` and `AgentCreateDialog` offer the `Latest` group; `storedIsDeclared` counts an alias; `runnerOptionLabel` renders an alias, if it exists
