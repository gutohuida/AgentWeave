...
Second of two changes; it builds on `a-flow-is-configured-from-its-own-tab`. **Nothing here is
implemented yet.**
...
- **The exploring interview asks "How will this be built?"** It nudges towards a flow: parallel
  ...
  condition and the cadence. "No flow" is a valid answer. *(operator)*
...
- **Approval creates the flow**, with the operator as the actor (so `allow_agent_jobs` never
  ...
  If the flow cannot be created (agent gone, document already claimed, bad cadence), **the document
  is still approved**, and the report says why. *(operator)* At approval the operator may pick a
...
- **Every approval writes an approval report:** tasks created, tasks skipped as already served,
  ...
  no flow was created, it offers change 1's **Start a flow…**.
...
- **Backend:**
  ...
  - `launchability.py` and `api/v1/agents.py`: interview text, plus a roster line in spec turns.
  ...
  - `api/v1/jobs.py`: the flow-creation body is extracted into a function both routes call.
- **UI:** `SpecPhaseBar.tsx` (the stale strip and the approve-time agent choice), a new
  `SpecApprovalReport.tsx` in `SpecDocumentPanel.tsx`, `api/spec.ts`, and a bundle refresh.
...
- **Order:** after change 1. Also after `a-document-moves-forward-only-through-its-checks` (B5: it
  ...
  Rebase the interview text onto `an-at-mention-an-agent-wrote-reads-no-file` and
  `a-specification-is-read-in-results-that-fit` if they land first; both edit the same lines.
