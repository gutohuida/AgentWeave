...
- [ ] 1.1 Find where the briefing's existing assertions live before writing new ones. Five test
  ...
  2,637 lines.
- [ ] 1.2 **F143's reproduction, and the cheap deterministic one.** Compose a briefing for a task
...
- [ ] 1.3 **F140's reproduction.** Compose an implementation briefing for a claimed task and assert
...
- [ ] 1.4 Assert the counterpart that must **stay** true, so the fix is not measured only by what it
...
- [ ] 2.1 Give `_compose_loop_briefing` a keyword-only `is_review: bool` with **no default**,
...
- [ ] 2.2 Pass it at both call sites: `scheduler.py:2362-2366` (the `_do_fire_job` path, where the
...
- [ ] 2.3 Update the six existing test call sites to state `is_review=False`. Do not give them a
  helper that defaults it — the explicitness is the point.
- [ ] 2.4 Extend the function's docstring with what the parameter decides. That docstring is already
...
- [ ] 3.1 After the tier paragraph and before `Purpose:`, and only when a task is claimed, state the
...
- [ ] 5.1 Under `is_review`, keep the tier paragraph, replace the *"Finish the task below and stop"*
  ...
  to build (design D4).
- [ ] 5.2 Name both verdicts with the calls that record them —
...
- [ ] 5.3 Do **not** restate what the context channel says about the detached checkout, running the
...
- [ ] 6.1 `scripts/drive/t_row12_review_leg.py` currently asserts F143's state — implementation
...
- [ ] 7.1 `py -3.11 -m pytest hub/tests/test_briefing_names_its_contract.py
