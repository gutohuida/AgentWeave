...
- [ ] 1.2a Control: the operator's `POST /projects/{p}/messages` to `operator` keeps today's `Unknown recipient` answer (the new sentence is addressed to an agent). PASSES today and must keep passing
...
- [ ] 2.1 `worktrees.is_reserved_agent_name`; the branch in `messages.py` before the recipient lookup, guarded by `not by_operator`
