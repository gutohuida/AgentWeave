...
- [ ] 0.4 The operator answers D12's third question (recommended: one spelling `""`, resolved, with a data migration) and approves (APPROVALS.md); the 2026-09-24 review's fixes are applied (0.5)
...
- [ ] 1.5d (D4 unnamed arm, operator review) Function-level, rows' `branch` set directly to `""`: (i) two accepted footprints naming commits X and Y on unrelated histories (neither contains the other), Y observed later → `merge_targets` answers **both** X and Y; FAILS today (`integration_targets` keys `""` like a branch, `task_integration.py:283-285`, and keeps only Y). (ii) commit P and its descendant Q, **P observed later** → `[Q]` only; FAILS today (`[P]`). (iii) the same commit twice → one target carrying the later row's `evidence_id`; control. (iv) `is_reachable_from` patched to answer `None` with (i)'s rows → both kept; FAILS today. (v) one row with `branch=None` and one with `""`, unrelated commits → both kept (`NULL` joins the unnamed arm)
...
- [ ] 2.4 (D4) `_accepted_targets` shared; the ancestry reduction in `merge_targets`' governed path — the named arm, and the unnamed arm (`""` and `None`) that drops only proper ancestors and keeps both on an unknown probe; `integration_targets` unchanged. The preview's governed path (`tasks.py:1122`) calls `merge_targets` with the resolved root, wrapped as its ungoverned path is (skip if the F141 change already did)
