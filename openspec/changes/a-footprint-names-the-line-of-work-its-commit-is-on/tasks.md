...
- [ ] 1.5 (D4) Two accepted footprints on one task branch: author at D (observed first), reviewer at D's parent C (observed later). `merge_targets(session, task, root)` → `[D]`. Must be staged after D2 (or with branches set directly on the rows). FAILS on today's reduction when both rows carry the task branch
...
- [ ] 2.3 (D3) The released-checkout answer
- [ ] 2.4 (D4) `integration_targets(…, root=None)`; `merge_targets` passes root
- [ ] 2.5 (D1) Migration `0106_…`; bump head assertions in `test_migrations.py` and `test_project_persistence.py`
