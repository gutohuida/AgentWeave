...
- [ ] 4.2 **Does the posture survive the hop?** Set a posture in the composer, have that agent
      message a peer, and confirm the peer's run carries it.
...
3. **The posture survives a handoff.** In the composer set Permissions to *Ask me*, send a turn, and
   have that agent `send_message` to a second agent.
   - *Expect:* the second agent's run also asks you. Before this change it silently reverted.
4. **Full access does not spread.** Set Permissions to *Full access*, then have that agent message a
   peer.
   - *Expect:* the peer's run does **not** get full access — it falls back to the default.
