...
- [ ] 1.1 `Project.main_branch` (nullable) + migration `0070`. Null means "not chosen", which is not
      an error and blocks nothing.
- [ ] 1.2 `detect_main_branch(root)` — reuses the existing `MAIN_BRANCH_NAMES` order to *suggest* a
      name. It returns a suggestion and never assigns one.
- [ ] 1.3 Operator route to read and set the project's main branch, rejecting a name the repository
      does not have.
- [ ] 1.4 **`requirement_evidence.MAIN_BRANCH_NAMES` keeps its current reporting behaviour
...
- [ ] 1.5 Surface the setting where a project is configured, offering the detected name for the
      operator to accept.

## 2. Mergeability in the gate

- [ ] 2.1 `integration_target(session, task)` — the commits to integrate: the newest `commit_sha`
...
- [ ] 2.2 `would_conflict(root, commit, main_branch)` wrapping the existing
      `worktrees._merge_tree_conflicts()`. Touches no working tree and no index.
- [ ] 2.3 `GateRefusal` gains a third list for unmergeable work, carrying the conflicting paths.
      `detail()` and `to_dict()` include it, and the existing `message` sentence still reads
      correctly for a caller that reads only that.
- [ ] 2.4 Called from inside `requirement_gate.evaluate` — **not** a second call site in
      `apply_transition`. B4 §3.1's "one function, one place" property must survive this change.
- [ ] 2.5 The conflict refusal fires **regardless of rigor**, including `sketch`. It is a claim about
      possibility, not about verification.
- [ ] 2.6 No conflict check where there is nothing to merge: no configured main branch, no accepted
      git footprint, or not a repository.

## 3. The merge

- [ ] 3.1 `integrate(root, commit, main_branch)` — `git merge` into the main branch in the primary
      checkout. **Never `push`, never contacts a remote**, asserted by a source-level test as well as
      behaviourally.
- [ ] 3.2 Preconditions, each returning a distinct stated reason rather than a silent skip: main
      branch configured; primary checkout clean; primary checkout on the main branch.
- [ ] 3.3 Wired into `apply_transition` on the move into `approved`, after the transition is
      recorded. **A merge failure never rolls the transition back** (D6).
- [ ] 3.4 Integration runs regardless of rigor (D5).
- [ ] 3.5 A project that is not a repository, or has no accepted git footprint, attempts nothing and
      raises nothing.

## 4. The record

- [ ] 4.1 `task_integrations` model + the same migration `0070` — task, project, commit, source
      branch, target branch, outcome (`merged`/`skipped`/`failed`), reason, mechanism (`local`),
      approving actor, time.
- [ ] 4.2 Append-only: no update path, no delete path, asserted by a source scan the way
      `spec_rigor_events` is.
- [ ] 4.3 `mechanism` is stored on every row so a later GitHub-based integration is distinguishable
      in history rather than conflated with this one (D8).
- [ ] 4.4 Route to read a task's integration history.

## 5. Navigation

- [ ] 5.1 A task shows whether its work was integrated, and where it went — or why it did not.
- [ ] 5.2 The gate refusal surface renders the unmergeable case with its conflicting paths, beside
      the unverified-requirement case it already renders.
- [ ] 5.3 Coverage surfaces are **not** changed. B3 already reports `verified, not integrated`; this
      change makes that state reachable in the other direction, and needs no new vocabulary for it.

## 6. Tests — agent-verifiable

- [ ] 6.1 Target selection: the newest accepted footprint commit wins; awaiting evidence contributes
...
- [ ] 6.3 Later commits on the agent's branch are **not** merged (D1) — the test that proves the
      commit-not-branch decision.
- [ ] 6.4 A `sketch` document's task integrates identically to a `gate` document's task (D5).
- [ ] 6.5 A conflict refuses approval at `sketch` rigor (D3's accepted consequence).
- [ ] 6.6 Each skip precondition, separately: no main branch; dirty checkout; checkout on another
...
- [ ] 6.8 A non-repository project approves exactly as it did before this change (D7).
- [ ] 6.9 A project with `main_branch` null reports integration exactly as it did before this change
      (task 1.4).
- [ ] 6.10 **Nothing pushes** — a source-level assertion plus a test with a configured remote that
      fails if the remote is touched.
- [ ] 6.11 `task_integrations` appends and never updates.
- [ ] 6.12 The gate is still one function called from one place — the B4 §3.1 source check still
      passes and is extended to cover the conflict path.
- [ ] 6.13 `test_migrations.py` and `test_project_persistence.py` head assertions bumped to `0070`;
      the migration guards for a missing table.
- [ ] 6.14 `pytest hub/tests/ -q` and `pytest tests/ -q` separately; `ruff`; `black
      --target-version py311`; `npx tsc --noEmit`; `npx openspec validate --changes --strict`.
- [ ] 6.15 `hub/hub/static/ui` refreshed and confirmed with `diff -rq`.
