...
| A5 | The startup line exists **and is at a level that shows** | Task 2.6 — assert on the `caplog` record's `levelno >= logging.WARNING`. A test asserting only the text passes on an invisible line, which is the exact defect D5 found in `engine.py:228`. **(R4)** This is the fast check only. The visibility claim itself is A17, because under `pytest` no alembic `fileConfig` ever runs (`:memory:`) |
