...
- [ ] 1.8 (D2, D5) Control: two different runs of the same agent at the same commit, checkout **clean** → the second is still refused `duplicate_evidence`. Passes before and after
...
- [ ] 2.1 (D1) `requirement_evidence.decide`: the liveness refusal after the value, grant and self-acceptance checks, immediately before the review is built. Import `run_liveness` at module scope (it imports only `db.models` and `pty_runner`; confirm no cycle)
- [ ] 2.2 (D2) `record`: the same-run, awaiting, same-digest revise branch, re-applying the footprint with `outside_writes` read from the run; return a marker the routes read (a `revised` attribute set on the instance, or a small result type — R2 picks)
