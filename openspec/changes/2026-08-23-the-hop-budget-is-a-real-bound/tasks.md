...
- [ ] 1.1 In `turn_scheduler.schedule_agent`, filter the selected batch to entries whose
      `hop_depth` is within the project's budget. Today the selection filters only by conversation
      and the delivery cap.
- [ ] 1.2 Set the turn's depth from the admitting entry rather than `min(hop_depth)` (design D2).
- [ ] 1.3 Tests: a batch mixing an in-budget and an over-budget entry delivers only the first; the
...
- [ ] 1.4 Regression test reproducing F5 exactly: budget 1, chain to depth 2, operator message into
...
- [ ] 4.2 `cd hub/ui && npm run lint && npx tsc --noEmit`, then `npm run build` and
      `python scripts/refresh_ui_bundle.py`. Re-stamp **after** the commit that normalises line
      endings, or `/health` reports `ui_stale` on a clean tree.
