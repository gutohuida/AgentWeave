...
- [ ] 0.1 R2: independent re-derivation against `hub/hub/api/v1/agents.py` (register, create, request, PATCH, detail, list), `hub/hub/launchability.py:470-520`, `hub/hub/scheduler.py:229-252,3078`, `hub/hub/db/models.py:193-222`, and every `agents/register` caller under `hub/tests/`, `scripts/`, `.claude/skills/`, `docs/`. Rebuild design D3's table from `grep` before reading it
...
- [ ] 1.3 Same file: an agent made by `add_agent` with no runner bound — `GET /agents/launchability` reports it with `runner == "unbound"` and a reason that does not contain the agent's name as a CLI; `POST /agent/trigger` answers queued with a `waiting_reason` that does not contain `Runner CLI '<name>'`. Written against a row with `self_registered=True` **before** the migration (the column still exists), it FAILS today — that is F136's reproduction, and it is the assertion that would fail if `launchability.py:507`'s guard came back
...
- [ ] 2.3 `launchability.py:507` → `elif "runner" not in meta:`; rewrite the comment block `:491-514` to say no agent is exempt by origin (and drop the claim about `collaboration_ready`)
...
- [ ] 2.10 Harnesses and docs: update or retire the 8 `scripts/drive/` callers and `.claude/skills/e2e-loop/e2e.py`; delete the route's row in `docs/reference/hub-api.md`
