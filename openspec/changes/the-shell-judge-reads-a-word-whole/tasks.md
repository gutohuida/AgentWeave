...
- [ ] 0.3 The operator approves the change in `spec-queue/APPROVALS.md` and answers design Open Questions 1 to 3; before `mcp_server.py` is edited, the operator is told that `:8000`'s next run is judged by the edited file, committed or not
...
- [ ] 1.4d (R4, bounds) A directory `big/` of 8193 empty files: `ls big/*` is refused with the too-many reason (FAILS against R3, which allows it). `ls sub/*` is allowed. With `globstar` named (`bash -O globstar -c 'ls **/x'`) a link two levels down (`sub/deep/l` → outside) is refused, and without it the same `ls **/x` is allowed (the named residual; assert it, so a change of mind is visible)
...
- [ ] 1.6 Totality. `_decide` answers, and never raises, for these `Bash` commands:
  ...
  (R4) A link cycle (`work/loop` → `work`): `ls loop/*/*/*/x` answers (refused past the budget or allowed, never raised).

  (R4) The memo: a Bash command read in both readings charges the budget once, so `ls sub/*` with a budget of exactly one listing is allowed. Monkeypatch the bound to prove it; this FAILS without the memo.
