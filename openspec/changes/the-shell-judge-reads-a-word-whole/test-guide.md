...
| **A link behind `@`, a quote or a colon is resolved** (R8) | same file (task 1.4g), with `sub/@s/p`, `a'b/up` → outside and `a@b/l` → the workspace | `sub/@s/p/x`, `"a'b/up/x"`, `-Destination:sub/@s/p/x`, `sub/@s/p/*`, `"a@b/l/../y"`, `"../work(a"` refused, naming where each resolves; `HEAD:src/a.py`, `print(1/2)`, `s/(foo)/\1/` and, on Windows, `'ORM\|:2580'` allowed |
| **A bracket is matched as the shell matches it** (R8) | same file (task 1.4c) | `./[0-9]/x` allowed beside the link `up`; `./[^a]p/x` and PowerShell `[!a]p` refused |
