...
- [ ] 1.6 (D4) `hub/ui/src/__tests__/useSSE.test.tsx`: a response whose chunks are, **in the order `stream_frames` emits them**, `event: connected`, one `task_updated` stamped with `project_id: "proj-a"`, then `event: stream_gap` / `data: {"dropped":10,"severity":"warn"}`. The test renders **exactly one** `useSSE()` hook (each mounted hook registers its own invalidate-everything reconnect listener, `useSSE.ts:410-414`, so the count below is exact only with one), and says so in a comment. Assert: a listener sees `stream_gap` after `task_updated`; `queryClient.invalidateQueries` is called with **no** arguments exactly once after the gap; an `onSseReconnect` callback fires once. FAILS today (the allowlist at `useSSE.ts:337` drops `stream_gap`)
- [ ] 1.7 (D4) The same stream with the gap placed **before** `task_updated` (the order D2 also permits), again with exactly one `useSSE()` mounted: the catch-up still fires once and `task_updated` is still dispatched. FAILS today
