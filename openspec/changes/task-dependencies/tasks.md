...
- [ ] 1.1 Add `depends_on: List[str]` to `spec_payload.Task`. Default empty — an existing document must validate unchanged.
- [ ] 1.2 Write the `Field(description=...)`. These **are** the agent-facing instructions — *"One concrete unit of work, not 'build the whole thing'"* is how decomposition is taught today, and ordering gets taught in the same place. Say that keys name siblings in this document.
- [ ] 1.3 Add the imported-entry shape: an entry naming another document's path and a task key in it, distinguishable from a locally-declared task. Decide whether that is a discriminator field on `Task` or a separate list, and record which in design D4 when you find out which reads better in a real payload.
- [ ] 1.4 Write its description too, including that the referenced document must be approved and why.
- [ ] 1.5 Round-trip test: a payload with dependencies and imports survives render → `extract_payload` → validate unchanged.
