...
- [ ] 2.3 `launchability.agent_config(session_data, agent_name, agent_config)` (R3: the whole merge, lifted from `get_agent_config` unchanged, not a fallback applied after it), used by `get_agent_config` and the list route; `AgentSummary.permission_mode_at_rest` (`hub/hub/schemas/agents.py`) and the list serializer (`hub/hub/api/v1/agents.py:595-615`)
- [ ] 2.4 UI: `api/agents.ts` type and `postureAtRest(agentRow)` (R3), read by `AgentOutputPanel.tsx:393-395` **and** `NewConversationSurface.tsx:201` (R3), and `AgentSettingsControls.tsx:199`; update `__tests__/support/modelCatalogFixture.ts` to the new Claude default
