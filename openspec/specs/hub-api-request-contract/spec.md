# hub-api-request-contract Specification

## Purpose

What the Hub's HTTP write surface promises a caller about the request they sent: that a field the
receiving contract cannot honour is refused by name rather than absorbed, uniformly across every
route, and that where a contract declines that rule it says so in its own text. Covers the
translation of superseded vocabularies, which is where the rule is most easily reintroduced as its
own failure, and the detection of a route that evades the rule by declaring no contract at all.
## Requirements
