...
A function that stages its event row inside a transaction (writes it with `commit=False`), whether
it leaves the commit to its caller or commits itself, SHALL announce through a deferred form that is
published by the commit of that same session's root transaction, and SHALL stage that announcement
before the commit it reports. A failure to publish a deferred announcement MUST NOT fail the commit
or change what the calling route answers.
