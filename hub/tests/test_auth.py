...
async def test_no_auth_returns_401(app):
    resp = await app.get("/api/v1/projects/proj-test/status")
...
async def test_bad_key_returns_401(app):
    resp = await app.get(
        "/api/v1/projects/proj-test/status", headers={"Authorization": "Bearer aw_live_wrong"}
    )
    assert resp.status_code == 401
...
async def test_query_token_rejected_on_non_sse_endpoints(app, auth_headers):
    ...
    resp = await app.get("/api/v1/projects/proj-test/status", params={"token": token})
    assert resp.status_code == 401
...
async def test_sse_ticket_flow(app, auth_headers):
    ...
    ticket_resp = await app.get("/api/v1/projects/proj-test/events/ticket", headers=auth_headers)
    ...
    try:
        await asyncio.wait_for(
            app.get("/api/v1/projects/proj-test/events", params={"token": token}),
            timeout=1.0,
        )
