...
**Why the default fixture project, not `proj-b44fac0c`.** `test_panel_shell.py` uses the
operator's disposable project because the default fixture project had no agent with a
conversation at the time it was written. That has since changed: the default project's own
`claude-1` agent now carries real conversations and real loops (`LB1`-`LB4`'s own live smoke
tests, left in place as historical evidence per their own precedent) — exactly the fixture this
suite needs and `proj-b44fac0c` does not have (it carries zero loops). Read from, never mutated.

Uses the `project_id` fixture (defaults to `proj-5e960453`, overridable via `AW_HUB_PROJECT_ID`)
rather than a hardcoded constant — unlike `proj-b44fac0c` above, this is the same "default
project" identity `conftest.py` already parameterizes, so there is no reason for this file to
...
def _open_bare(page: Page, hub_url: str, project_id: str) -> Page:
...
def _open_loops_tab(page: Page, hub_url: str, project_id: str) -> Page:
    _open_bare(page, hub_url, project_id)
...
def test_the_loops_tab_is_reachable_from_a_bare_conversation(
...
def test_loops_index_lists_real_loops_with_counts_by_ending_state(
    page: Page, hub_url: str, project_id: str
) -> None:
    ...
    _open_loops_tab(page, hub_url, project_id)
...
def test_loop_drill_down_shows_the_claimed_item_and_queue(
    page: Page, hub_url: str, project_id: str
) -> None:
