...
**Why `spec_project_id`, not this suite's usual `project_id`.** A conversation destination needs
a real agent, and the default fixture project has none — `test_command_palette.py` already
documents this as a standing gap. `spec_project_id` (`proj-b44fac0c`, "Throwaway (taste pass)",
overridable with `AW_HUB_SPEC_PROJECT_ID`) is the operator's own disposable project and already
carries a real agent (`q2verify`) with a real conversation and a real specification document, so
it is read from, never mutated, here.
"""
...
def spec_tab_id(api, spec_project_id: str) -> str:
    ...
    status, body = api("GET", f"/api/v1/projects/{spec_project_id}/project/specs")
...
def _open(page: Page, hub_url: str, spec_project_id: str, *, width: int | None = None) -> Page:
    if width is not None:
        page.set_viewport_size({"width": width, "height": 900})
    query = (
        f"project={spec_project_id}&agent={AGENT}&conversation={CONVERSATION_ID}"
...
def test_the_document_opens_inside_the_shell_not_as_a_one_off_block(
    page: Page, hub_url: str, spec_project_id: str, spec_tab_id: str
) -> None:
...
def test_narrow_window_overlays_the_shell_and_keeps_it_on_dismiss(
    ...
    """
    _open(page, hub_url, spec_project_id, width=700)
...
def test_the_composers_own_close_control_also_closes_the_shell(
...
def test_opening_a_document_from_a_bare_conversation_mounts_the_shell(
    ...
    query = f"project={spec_project_id}&agent={AGENT}&conversation={CONVERSATION_ID}"
    page.goto(f"{hub_url}/?{query}", wait_until="load")
...
def test_wide_window_lays_out_as_two_columns(
    page: Page, hub_url: str, spec_project_id: str
) -> None:
    ...
    _open(page, hub_url, spec_project_id, width=1440)
