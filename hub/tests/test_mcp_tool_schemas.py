...
# ---------------------------------------------------------------------------
# F35, reversed by the operator 2026-08-25
# ---------------------------------------------------------------------------
...
def test_the_structured_fields_advertise_their_shape():
    ...
    `{"anyOf": [{}, {"type": "null"}]}`, which advertises no shape at all. The operator chose the
    schema. This is what holds that choice: an `Any` slipped back in here would make the refusal
    machinery necessary again without anyone noticing it had gone.
    """
    schema = _schemas()["submit_spec_document"]
    wanted = {
        "scope": "object",
