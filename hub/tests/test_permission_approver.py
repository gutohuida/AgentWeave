...
class _WrongAnswerError(AssertionError):
    """The decision allowed what the row refuses, or refused what it allows."""


class _WrongReasonError(AssertionError):
...
def _outside_on_windows(word: str) -> tuple:
    """POSIX reads `\\` as an ordinary character, so the word is a name written inside; Windows
    reads it as a separator, so the same word is a traversal."""
    return (False, _outside(word)) if _WINDOWS else (True, None)
...
def test_a_quote_is_judged_by_what_it_decodes_to(workspace, monkeypatch, command, allow, reason):
    monkeypatch.setenv("HUB_URL", _HUB)
    decision = _decide("Bash", {"command": command})
    if decision["allow"] is not allow:
        raise _WrongAnswerError(decision["reason"])
    said = decision["reason"]
    if reason == _UNCHECKED:
        if "cannot be checked" not in said or "outside your workspace" in said:
