...
async def test_agent_context_includes_bound_charter(app, auth_headers):
    ...
    response = await app.get("/api/v1/agents/agent-context?agent=chartered", headers=auth_headers)
...
async def test_agent_without_charter_gets_instructions_and_notice(app, auth_headers):
    ...
    response = await app.get("/api/v1/agents/agent-context?agent=unchartered", headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
...
async def test_direct_charter_lookup_rejects_unknown_id(app, auth_headers):
    response = await app.get(
        "/api/v1/agents/context?charter=charter-does-not-exist", headers=auth_headers
    )
