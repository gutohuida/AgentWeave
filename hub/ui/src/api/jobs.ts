...
export interface LoopSummary {
  ...
  /** `agent_capacity` says what `agent` means, which the board previously left the reader to infer
   ...
   *  Named `agent_capacity` rather than `agent_role` because `agent-loops` calls the concept
   *  capacity, while "role" already means a charter, a turn kind, and a deleted subsystem. */
  current_tasks?: {
    id: string
    title: string
    status: string
    agent?: string
    agent_capacity?: 'working' | 'held' | 'next' | 'assigned'
  }[]
