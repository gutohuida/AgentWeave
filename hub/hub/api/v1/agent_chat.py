...
from ...db.models import AgentOutput, InboundQueueEntry, Message, Project, Run
...
async def _queued_entries_for(
    session: AsyncSession, project_id: str, agent: str, hop_budget: Optional[int]
) -> List[TimelineEntry]:
...
async def get_chat_history(
    ...
    """Full merged timeline for one session, plus this agent's current queue."""
    ...
    return ChatHistoryResponse(session_id=session_id, agent=agent, entries=entries)
...
async def get_recent_chat(
    ...
    return ChatHistoryResponse(session_id=None, agent=agent, entries=entries)
